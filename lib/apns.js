...
// APNS_KEY_P8 is the .p8 PEM contents. It MUST be single-line on Coolify
// because Coolify injects env vars as Dockerfile ARG instructions at build
// time, and `ARG FOO='multi\nline'` blows up Docker's parser (see lesson
// IOS_TESTFLIGHT_AUTONOMY.md). So we accept EITHER:
//   - the raw PEM (with real newlines), OR
//   - a base64-encoded PEM (no newlines, opaque), via APNS_KEY_P8_BASE64.
// The base64 form is what we recommend on Coolify.
function resolveP8() {
  const raw = process.env.APNS_KEY_P8 || '';
  if (raw && raw.includes('BEGIN')) return raw;
  const b64 = process.env.APNS_KEY_P8_BASE64 || raw;
  if (!b64) return '';
  try {
