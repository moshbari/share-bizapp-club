...
const M = {
  insert: db.prepare(`
    INSERT INTO messages (slug, user_id, title, body)
    VALUES (?, ?, ?, ?)
  `),
  getBySlug: db.prepare(`
    SELECT id, slug, user_id, title, body, created_at, updated_at
  ...
  `),
  // Filtered list — q searches title OR body (LIKE)
...
const messages = {
  insert({ slug, userId, title, body }) {
    M.insert.run(slug, userId, (title || '').slice(0, 200), body || '');
  },
   ...
   * id from the previous page (cursor).
   */
...
module.exports = { users, files, passwordResets, magicLinks, messages, raw: db };
