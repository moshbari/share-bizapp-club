...
const F = {
  ...
  claimPendingForGuest: db.prepare(`
    ...
    WHERE guest_id = ? AND activated = 0
  `),
...
const files = {
  ...
  /**
   * Claim all pending files tied to guestId for this user. Returns the
   * number of files activated.
   */
  claimPendingForGuest(userId, guestId) {
    return F.claimPendingForGuest.run(userId, guestId).changes;
  },
