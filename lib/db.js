...
// Notes travel with the file for every kind — a prompt shared under the
// video that demonstrates it, the caption under a screenshot, the source
// text behind a PDF. The viewer page shows them with a Copy button so the
// person you sent the link to can lift the text without asking you for it.
maybeAddColumn('files', 'notes', "TEXT NOT NULL DEFAULT ''");

// Drag-to-reorder ranking for the "Your recent shares" list. Higher
// sort_order = higher in the list. Existing rows are backfilled with
// sort_order = id, which reproduces the old newest-first order exactly,
// so nobody's list jumps around on the deploy that adds this.
maybeAddColumn('files', 'sort_order', 'INTEGER NOT NULL DEFAULT 0');
db.prepare(`UPDATE files SET sort_order = id WHERE sort_order = 0`).run();
db.exec(`CREATE INDEX IF NOT EXISTS idx_files_user_sort ON files(user_id, sort_order DESC)`);
...
// Same notes field as `files` — a chat scroll gets one too, so the prompt
// that produced the conversation can ride along with the screenshots.
maybeAddColumn('chats', 'notes', "TEXT NOT NULL DEFAULT ''");

// Same drag-to-reorder ranking for the chat-scroll list on /chats.
maybeAddColumn('chats', 'sort_order', 'INTEGER NOT NULL DEFAULT 0');
db.prepare(`UPDATE chats SET sort_order = id WHERE sort_order = 0`).run();
db.exec(`CREATE INDEX IF NOT EXISTS idx_chats_user_sort ON chats(user_id, sort_order DESC)`);
...
const F = {
  // sort_order = current table MAX + 1, so a fresh upload always lands at
  // the top of its owner's list. Table-wide (not per-user) because guest
  // uploads have no user_id yet and still need a sane rank once claimed.
  insert: db.prepare(`
    INSERT INTO files (slug, title, original_filename, kind, mime_type, size_bytes, download_allowed, ghl_url, user_id, activated, guest_id, pending_email, notes, sort_order)
    VALUES (@slug, @title, @original_filename, @kind, @mime_type, @size_bytes, @download_allowed, @ghl_url, @user_id, @activated, @guest_id, @pending_email, @notes,
            (SELECT COALESCE(MAX(sort_order), 0) + 1 FROM files))
  `),
  ...
  getBySlug: db.prepare(`
    SELECT id, slug, title, original_filename, kind, mime_type, size_bytes, download_allowed, ghl_url, created_at, user_id,
           activated, guest_id, pending_email, notes
    FROM files WHERE slug = ?
  `),
  listPageByUser: db.prepare(`
    SELECT id, slug, title, original_filename, kind, mime_type, size_bytes, download_allowed, ghl_url, created_at, user_id, notes, sort_order
    FROM files WHERE user_id = ? AND activated = 1 AND sort_order < ?
  ...
  updateNotes: db.prepare(`UPDATE files SET notes = ? WHERE slug = ? AND user_id = ?`),
  ...
  listPageAll: db.prepare(`
    SELECT id, slug, title, original_filename, kind, mime_type, size_bytes, download_allowed, ghl_url, created_at, user_id, notes
    FROM files WHERE id < ?
    ORDER BY id DESC LIMIT ?
  `),
  backfillNullUser: db.prepare(`UPDATE files SET user_id = ? WHERE user_id IS NULL`),
  getRankBySlugForUser: db.prepare(`
    SELECT id, sort_order FROM files WHERE slug = ? AND user_id = ? AND activated = 1
  `),
  setSortOrder: db.prepare(`UPDATE files SET sort_order = ? WHERE id = ?`),
};

const FIRST_PAGE_CURSOR = Number.MAX_SAFE_INTEGER;

// Notes hold pasted prompts, so the cap has to be generous — but it is a
// cap, because the whole body is inlined into every viewer page render.
const NOTES_MAX_CHARS = 20000;

/**
 * Apply a drag-and-drop reorder without disturbing rows the client never
 * saw. `rows` is the dragged set in its new top-to-bottom order, each
 * { id, sort_order }. We collect the sort_order slots those rows already
 * occupy, hand them out highest-first in the new order, and leave every
 * other row untouched — so reordering a filtered view, or the first page
 * of a long list, can't shuffle anything below it.
 */
function applyRankOrder(rows, setStmt) {
  if (rows.length < 2) return 0;
  const slots = rows.map(r => r.sort_order).sort((a, b) => b - a);
  let updated = 0;
  db.transaction(() => {
    rows.forEach((r, i) => {
      if (r.sort_order === slots[i]) return;
      updated += setStmt.run(slots[i], r.id).changes;
    });
  })();
  return updated;
}
...
const files = {
  insert(row) {
    F.insert.run({
      ...
      notes: (row.notes || '').toString().slice(0, NOTES_MAX_CHARS),
    });
  },
  ...
  listRecentFilteredByUser(userId, { before, limit = 10, kind, q, from, to } = {}) {
    ...
    if (q) {
      // Matches the search text anywhere in title, original_filename, or
      // the notes body — a saved prompt is often the only thing you
      // remember about the file. Case-insensitive via SQLite's LIKE
      // default (ASCII case fold).
      conds.push('(title LIKE ? OR original_filename LIKE ? OR notes LIKE ?)');
      const like = '%' + q + '%';
  ...
  updateNotes(slug, userId, notes) {
    return F.updateNotes.run((notes || '').toString().slice(0, NOTES_MAX_CHARS), slug, userId).changes > 0;
  },
  setDownloadAllowed(slug, userId, allowed) {
    return F.updateDownloadFlag.run(allowed ? 1 : 0, slug, userId).changes > 0;
  },
  /**
   * Rewrite the list order from an array of slugs in the desired
   * top-to-bottom order. Slugs the caller doesn't own are ignored.
   */
  reorderForUser(userId, slugs) {
    if (!Array.isArray(slugs)) return { ok: false, updated: 0 };
    const rows = [];
    const seen = new Set();
    for (const raw of slugs.slice(0, 200)) {
      const slug = (raw || '').toString();
      if (!slug || seen.has(slug)) continue;
      seen.add(slug);
      const row = F.getRankBySlugForUser.get(slug, userId);
      if (row) rows.push(row);
    }
    return { ok: true, updated: applyRankOrder(rows, F.setSortOrder) };
  },
...
// ---------- search ----------
//
// One search box has to find a note by anything the person remembers about
// it — a word in the title, a phrase buried in the body, or the name of the
// folder they filed it under. So every term is matched against all three,
// and every term has to hit something (AND, not OR): typing more words
// narrows the list the way people expect, instead of flooding it.
//
// LIKE '%term%' rather than FTS5 on purpose: substring matching is what
// makes a half-remembered word work ("banner" finds "banners", "ব্যানার"
// inside a longer Bangla word), it needs no index to keep in sync on every
// write, and at this data size (thousands of notes) it is instantaneous.

const SEARCH_TERM_LIMIT = 6;

/** Split a query into terms, escaping LIKE wildcards in each. */
function searchTerms(q) {
  return (q || '')
    ...
    .map((t) => t.slice(0, 100).replace(/[\\%_]/g, (ch) => '\\' + ch));
}

const messages = {
  insert({ slug, userId, user_id, title, body }) {
    // Same both-spellings rule + guard as groups.insert below; this row has
    // the identical NOT NULL owner column and the identical trap.
    // The owner appears twice — once as the FK and once inside the
    // MAX(sort_order) subquery so the new row goes to the top of
    // *this user's* list, not globally.
    const owner = userId ?? user_id;
    if (owner == null) throw new Error('messages.insert: userId (or user_id) is required');
    M.insert.run(slug, owner, (title || '').slice(0, 200), body || '', owner);
  },
  ...
  listRecentByUser(userId, { before, limit = 20, q } = {}) {
    ...
    // The ceiling used to be 100, which silently truncated both callers that
    // ask for more: the web feed (200) and the phone's full list. Nothing
    // said so — the list just ended.
    const cap = Math.max(1, Math.min(500, limit | 0 || 20));
    const needle = q ? '%' + q + '%' : '';
    const qParam = q || '';
    return M.listFiltered.all(userId, cursor, qParam, needle, needle, cap);
  },
  /**
   * Search EVERY note the user owns — the ones loose in the feed and the
   * ones filed inside folders, which the old list-with-a-q search skipped
   * entirely (it carried `group_id IS NULL`, so anything organised became
   * unfindable, which is exactly backwards).
   *
   * Each row comes back with its folder's slug + title so a result can say
   * where it lives. Notes whose TITLE matched sort first — if you typed the
   * title you want that note, not the twelve notes that mention it.
   */
  search(userId, q, { limit = 60 } = {}) {
    const terms = searchTerms(q);
    if (terms.length === 0) return [];
    const cap = Math.max(1, Math.min(200, limit | 0 || 60));

    const conds = ['m.user_id = ?'];
    const params = [userId];
    for (const t of terms) {
      const like = '%' + t + '%';
      conds.push(
        "(m.title LIKE ? ESCAPE '\\' OR m.body LIKE ? ESCAPE '\\' OR COALESCE(g.title, '') LIKE ? ESCAPE '\\')"
      );
      params.push(like, like, like);
    }
    // Title relevance: how many of the terms are in the title.
    const titleScore = terms.map(() => "(CASE WHEN m.title LIKE ? ESCAPE '\\' THEN 1 ELSE 0 END)").join(' + ');
    const scoreParams = terms.map((t) => '%' + t + '%');

    const sql = `
      SELECT m.id, m.slug, m.title, m.body, m.sort_order, m.group_id,
             m.group_position, m.created_at, m.updated_at,
             g.slug AS group_slug, g.title AS group_title,
             (${titleScore}) AS title_score
      FROM messages m
      LEFT JOIN message_groups g ON g.id = m.group_id
      WHERE ${conds.join(' AND ')}
      ORDER BY title_score DESC, m.sort_order DESC
      LIMIT ?
    `;
    return db.prepare(sql).all(...scoreParams, ...params, cap);
  },
...
const MG = {
  ...
  `),
  // Owner-less lookup, for the public /g/<slug> page. Same trust model as
  // /f/ and /m/: the slug is a random nanoid(8), so holding the link IS the
  // permission — nothing here is enumerable.
  getBySlug: db.prepare(`
    SELECT id, slug, user_id, title, sort_order, created_at, updated_at
    FROM message_groups WHERE slug = ?
  `),
...
const groups = {
  // Takes the owner as `userId` OR `user_id`: this file carries both spellings
  // (messages/groups grew up camelCase, files/chats snake_case) and the /api/v1
  // route passed the other one for months. A missing owner used to bind as NULL
  // and surface as "NOT NULL constraint failed" from deep inside SQLite, which
  // the route turned into a flat 400 — so the iOS "New folder" button failed
  // with "Could not save" and nothing said why. Fail loudly instead.
  insert({ slug, userId, user_id, title }) {
    const owner = userId ?? user_id;
    if (owner == null) throw new Error('groups.insert: userId (or user_id) is required');
    MG.insert.run(slug, owner, (title || '').slice(0, 200), owner);
  },
  getBySlugForUser(slug, userId) { return MG.getBySlugForUser.get(slug, userId) || null; },
  getBySlug(slug) { return MG.getBySlug.get(slug) || null; },
  ...
  /**
   * Folders whose own title matches every term. A folder hit is worth
   * showing whole — "brand kit" should hand back the folder and everything
   * in it, not just the notes that happen to repeat the words.
   */
  search(userId, q, { limit = 20 } = {}) {
    const terms = searchTerms(q);
    if (terms.length === 0) return [];
...
const C = {
  // sort_order = current MAX for this user + 1, so a new scroll lands at
  // the top of /chats until the owner drags it somewhere else.
  insert: db.prepare(`
    INSERT INTO chats (slug, user_id, title, status, download_allowed, sort_order)
    VALUES (@slug, @user_id, @title, @status, @download_allowed,
            (SELECT COALESCE(MAX(sort_order), 0) + 1 FROM chats WHERE user_id = @user_id))
  `),
  ...
  listByUser: db.prepare(`
    ...
    ORDER BY c.sort_order DESC LIMIT ? OFFSET ?
  `),
  ...
  setNotes: db.prepare(`UPDATE chats SET notes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`),
  ...
  `),
  getRankBySlugForUser: db.prepare(`
    SELECT id, sort_order FROM chats WHERE slug = ? AND user_id = ? AND status = 'ready'
  `),
  setSortOrder: db.prepare(`UPDATE chats SET sort_order = ? WHERE id = ?`),
};
...
const CI = {
  ...
  replaceForChat: db.prepare(`
    UPDATE chat_items
       SET ghl_url = @ghl_url, original_filename = @original_filename,
...
const chats = {
  create({ slug, user_id, title = '', download_allowed = 0, notes = '' }) {
    ...
    if (notes) C.setNotes.run(notes.toString().slice(0, NOTES_MAX_CHARS), id);
    return id;
  },
  ...
  setNotes(id, notes) { return C.setNotes.run((notes || '').toString().slice(0, NOTES_MAX_CHARS), id).changes > 0; },
  ...
  /**
   * Swap the picture behind one item, keeping its id and its place in the
   * scroll — this is an EDIT of screenshot 3, not a new screenshot 7. Returns
   * the row as it was, so the caller can delete the picture it replaced.
   */
  replaceItem(chatId, itemId, row) {
    const before = CI.getForChat.get(itemId, chatId);
    if (!before) return null;
    CI.replaceForChat.run({
      id: itemId,
      chat_id: chatId,
      ghl_url: row.ghl_url,
      original_filename: (row.original_filename || before.original_filename || '').slice(0, 255),
      mime_type: row.mime_type || before.mime_type,
      size_bytes: row.size_bytes || 0,
      width: row.width || 0,
      height: row.height || 0,
    });
  ...
  /**
   * Reorder the chat-scroll cards on /chats from an array of slugs in the
   * desired top-to-bottom order. Same slot-reuse rule as files.
   */
  reorderForUser(userId, slugs) {
    if (!Array.isArray(slugs)) return { ok: false, updated: 0 };
    const rows = [];
    ...
    for (const raw of slugs.slice(0, 200)) {
      ...
      if (!slug || seen.has(slug)) continue;
      seen.add(slug);
      const row = C.getRankBySlugForUser.get(slug, userId);
      if (row) rows.push(row);
    }
    return { ok: true, updated: applyRankOrder(rows, C.setSortOrder) };
  },
...
module.exports = { users, files, passwordResets, magicLinks, messages, groups, feed, chats, apiTokens, deviceTokens, NOTES_MAX_CHARS, raw: db };
