...
function saveVsl(page, body, { getFile, ownHosts }) {
  ...
  // Fields left out keep what the page already has (the phone may send only `enabled`).
  const title = (body.title != null ? body.title : page.vsl_title || '').toString().trim().slice(0, 140);
  const position = body.position != null ? (body.position === 'top' ? 'top' : 'below_bio') : (page.vsl_position || 'below_bio');
