...
// GHL's media API only accepts these audio formats on the upload
// endpoint — everything else in GHL_AUDIO_EXT gets transcoded to mp3
// via ffmpeg before upload (iPhone voice memos are .m4a, so this hits
// every single time someone records on their phone).
const GHL_AUDIO_UPLOAD_OK = new Set(['mp3', 'wav', 'ogg', 'oga']);
...
// GHL media-upload API caps (NOT the GHL UI caps, which are higher).
// Source: https://marketplace.gohighlevel.com/docs/ghl/medias/upload-media-content
//   "If adding a file, maximum allowed is 25 MB. For video files,
//    the maximum allowed size is 500 MB."
// The UI in the GHL dashboard accepts up to 100 MB / 4 GB, but the
// `/medias/upload-file` POST endpoint we use rejects anything larger
// than these limits with a 413-ish error. Keep the app's caps in sync
// with the API or uploads will pass client validation and then fail
// server-side.
const SIZE_CAPS = {
...
function classify(originalName, browserMime) {
  ...
  if (m.startsWith('audio/') || GHL_AUDIO_EXT.has(ext)) {
    const needsTranscode = !GHL_AUDIO_UPLOAD_OK.has(ext);
    if (needsTranscode) {
      // ffmpeg pipeline upstream will convert to mp3 — pin the GHL
      // upload MIME and extension accordingly so the media-library
      // entry looks native.
      return {
        kind: 'audio',
        mime: 'audio/mpeg',
