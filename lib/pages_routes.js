...
<body class="pg th-${escHtml(page.theme)}" data-handle="${escHtml(page.handle)}">
  ...
  /** { block } or { error } */
  function addBlockFor(owner, body) {
  ...
  function storeAvatar(owner, file) {
  ...
  app.post('/page/avatar', requireUser, upload.single('file'), (req, res) => {
    try {
      storeAvatar(udb.getById(req.user.id), req.file);
      res.redirect('/page?pic=1');
    } catch (err) {
      res.redirect('/page?err=' + encodeURIComponent(err.message));
    } finally {
  ...
  app.post('/api/page/blocks', requireUser, express.json({ limit: '16kb' }), (req, res) => {
    const owner = udb.getById(req.user.id);
    const out = addBlockFor(owner, req.body || {});
  ...
  if (requireApiToken) {
    ...
    router.get('/page', requireApiToken, (req, res) => res.json(editorJSON(me(req))));

    router.patch('/page', requireApiToken, json, (req, res) => {
      const owner = me(req);
      const page = ensurePage(owner);
      const b = req.body || {};
      const problem = pages.updateProfile(page, {
      ...
      res.json(editorJSON(owner));
    });

    router.post('/page/avatar', requireApiToken, upload.single('file'), (req, res) => {
      try {
        const owner = me(req);
        storeAvatar(owner, req.file);
        res.json(editorJSON(owner));
      } catch (err) {
    ...
    router.get('/page/choices', requireApiToken, (req, res) => {
      const owner = me(req);
      res.json({
        ok: true,
    ...
    router.post('/page/blocks', requireApiToken, json, (req, res) => {
      const owner = me(req);
      const out = addBlockFor(owner, req.body || {});
      if (out.error) return res.status(400).json({ ok: false, error: out.error });
      res.json(editorJSON(owner));
    ...
    // Registered before '/page/blocks/:slug' so "reorder" is never read as a slug.
    router.post('/page/blocks/reorder', requireApiToken, json, (req, res) => {
      const owner = me(req);
      const page = ensurePage(owner);
    ...
    router.patch('/page/blocks/:slug', requireApiToken, json, (req, res) => {
      const found = ownBlockV1(req, res);
      if (!found) return;
      const out = patchBlock(found.b, req.body || {});
      if (out.error) return res.status(400).json({ ok: false, error: out.error });
      res.json(editorJSON(found.owner));
    });

    router.delete('/page/blocks/:slug', requireApiToken, (req, res) => {
      const found = ownBlockV1(req, res);
      if (!found) return;
      pages.deleteBlock(found.b);
      res.json(editorJSON(found.owner));
    });
