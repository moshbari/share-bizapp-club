...
// ---------- NOTES ----------
//
// Every kind of share can carry notes: the prompt demonstrated in the
// video, the caption behind a screenshot, the snippet a PDF explains. The
// point is that the person you sent the link to can COPY it — so the block
// is a plain <pre> (exact whitespace, no markdown mangling) plus one copy
// button. The text is also stashed in a JSON island so the button copies
// the real characters rather than whatever the browser did to the markup.
//
// `idPrefix` keeps the ids unique on pages that already have a copy button
// of their own (the text viewer's "Copy" for the file body).
function renderNotes(notes, { idPrefix = 'notes', heading = '📝 Notes' } = {}) {
  const body = (notes == null ? '' : String(notes)).trim();
  if (!body) return '';
  const dataId = `${idPrefix}-data`;
  const copyId = `${idPrefix}-copy`;
  // JSON in a <script> island must NOT be HTML-escaped: script content is
  // raw text, so `&quot;` would survive verbatim into textContent and blow
  // up JSON.parse. Escaping `<` as the JSON escape < is what makes it
  // safe — it cannot spell `</script>`, and JSON.parse turns it back into
  // a literal `<`.
  const rawJson = JSON.stringify(body).replace(/</g, '\\u003c');
  return `
    <div class="card notes-card">
      <div class="notes-head">
        <span class="notes-title">${escHtml(heading)}</span>
        <button type="button" class="btn btn-secondary btn-sm notes-copy" id="${copyId}">Copy</button>
      </div>
      <pre class="notes-body">${escHtml(body)}</pre>
    </div>
    <script id="${dataId}" type="application/json">${rawJson}</script>
    <script>
      (function () {
        var txt = '';
        try {
          var node = document.getElementById(${JSON.stringify(dataId)});
          if (node) txt = JSON.parse(node.textContent);
        } catch (e) { txt = ''; }
        var btn = document.getElementById(${JSON.stringify(copyId)});
        if (!btn) return;
        btn.addEventListener('click', async function () {
          ...
          try {
            ...
            // Clipboard API needs a secure context and can be blocked in
            // embedded webviews. Selecting the text is the fallback that
            // always works — the reader just hits ⌘C / Ctrl-C.
            var pre = btn.closest('.notes-card').querySelector('.notes-body');
            try {
...
function renderText(rec, bodyText, siteName) {
  ...
  // Stash the raw text in a JSON island so the source/copy buttons can pull
  // from the DOM instead of needing a fetch round-trip. See renderNotes for
  // why this is escaped as JSON only — HTML-escaping it breaks JSON.parse,
  // because character references are never decoded inside <script>.
  const rawJson = JSON.stringify(bodyText).replace(/</g, '\\u003c');
  ...
  return `
    ...
    ${renderNotes(rec.notes)}
    <div class="footer">Shared from ${escHtml(siteName)}</div>
    <script id="raw-text-data" type="application/json">${rawJson}</script>
    <script>
      (function () {
        ...
        try {
          ...
          if (node) raw = JSON.parse(node.textContent);
        } catch (e) { raw = ''; }
...
module.exports = { renderImage, renderVideo, renderAudio, renderPdf, renderText, renderNotes, escHtml, fmtBytes };
