...
const P = {
  getByUser: db.prepare(`SELECT * FROM pages WHERE user_id = ?`),
...
const pages = {
  ...
  create(user) {
    const handle = suggestHandle(user.name, user.email);
    P.insert.run({
      user_id: user.id,
      handle,
      display_name: (user.name || '').slice(0, 80),
      bio: '',
      theme: 'sunrise',
    });
    return P.getByUser.get(user.id);
  },
