...
function groupToJSON(g, children, publicOrigin) {
  ...
  return {
    ...
    // The folder's own shareable page. The server owns this string because
    // it knows which domain is the public one — the app is pinned to the
    // older hostname and would otherwise hand out a link on the wrong domain.
    public_url: `${publicOrigin}/g/${g.slug}`,
...
function attach(app, deps) {
  ...
  // So routes mounted by other modules (lib/pages_routes.js) authenticate the
  // phone exactly the same way, instead of carrying a second copy of this.
  attach.requireApiToken = requireApiToken;
  ...
  router.get('/messages', requireApiToken, (req, res) => {
    ...
    const allMessages = mdb.listRecentByUser(req.user.id, { limit: 500 });
  ...
  // Search every note the person owns — loose ones and the ones filed in
  // folders — plus folders by name. The phone filters what it already holds
  // for instant feedback; this exists so a search still finds notes the
  // list hasn't paged in, and so both platforms agree on what "matches"
  // means (every word must appear, in the title, the text, or the folder
  // name). No index, no AI: it is the person's own words, matched literally.
  router.get('/messages/search', requireApiToken, (req, res) => {
    const q = (req.query.q || '').toString().trim().slice(0, 100);
    if (!q) return res.json({ ok: true, q: '', messages: [], groups: [] });
    const hits = mdb.search(req.user.id, q, { limit: 100 });
    const groupHits = gdb.search(req.user.id, q, { limit: 20 });
    res.json({
      ok: true,
      q,
      messages: hits.map((m) => ({
        ...messageToJSON(m, PUBLIC_ORIGIN),
        group_slug: m.group_slug || null,
        group_title: m.group_title || null,
      })),
      groups: groupHits.map((g) => groupToJSON(g, mdb.listInGroup(g.id, req.user.id), PUBLIC_ORIGIN)),
    });
  });
  ...
  router.post('/groups', requireApiToken, jsonBody, (req, res) => {
    try {
      const slug = nanoid(8);
      gdb.insert({ slug, userId: req.user.id, title: (req.body?.title || '').toString().slice(0, 200) });
      const fresh = gdb.getBySlugForUser(slug, req.user.id);
      res.json({ ok: true, group: groupToJSON(fresh, [], PUBLIC_ORIGIN) });
    } catch (err) {
      // This used to swallow a real server-side fault as a 400 with no log
      // line, so "Could not save. Try again." was all anyone ever saw. A
      // failure here is ours, not the client's: log it and say so.
      console.error('[api/v1 POST /groups]', err);
      res.status(500).json({ ok: false, error: 'Could not make the folder.' });
    }
  });
